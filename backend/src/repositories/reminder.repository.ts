...
export const reminderRepository = {
  async createEvent(data: any) {
    return prisma.reminderEvent.create({ data });
  },

  async updateEvent(id: string, data: any) {
    return prisma.reminderEvent.update({ where: { id }, data });
  },
  ...
  async createReminders(dataArray: any[]) {
    return prisma.reminder.createMany({ data: dataArray });
  },
  ...
  async findPendingReminders(userId: string) {
    ...
    return prisma.reminder.findMany({
      ...
      include: {
        event: { include: { client: true, cas: true } },
      },
  ...
  async getRemindersByEventId(eventId: string) {
    return prisma.reminder.findMany({
      where: { eventId },
      orderBy: { remindAt: "asc" },
    });
  ...
  async deleteRemindersByEventId(eventId: string) {
