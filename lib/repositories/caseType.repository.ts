...
export const caseTypeRepository = {
  async findAll(params: { onlyActive?: boolean } = {}) {
    ...
    return prisma.caseType.findMany({ where, include: { documents: { orderBy: { order: "asc" } }, _count: { select: { cases: true } } }, orderBy: { nameAr: "asc" } });
  },
  async findById(id: string) {
    return prisma.caseType.findUnique({ where: { id }, include: { documents: { orderBy: { order: "asc" } }, _count: { select: { cases: true } } } });
  },
